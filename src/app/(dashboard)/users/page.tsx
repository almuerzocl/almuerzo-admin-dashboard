...
export default function UsersAdmin() {
    ...
    const handleCreateUser = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                setNewUserData({ first_name: '', last_name: '', email: '', password: '', role: 'user' });
            }
        } catch (err: any) {
    ...
    const renderTable = (type: 'pwa' | 'restaurant' | 'admin', users: ExtendedUser[]) => {
        ...
        return (
            <div className="bg-zinc-950/50 border border-zinc-800 rounded-[2rem] overflow-hidden overflow-x-auto mt-6">
                <table className="w-full text-left">
                    <thead>
                        <tr className="border-b border-zinc-800 bg-zinc-900/50">
                            <th className="px-6 py-4 text-[10px] font-black text-zinc-500 uppercase tracking-widest w-24 text-center">Activo</th>
                            <th className="px-6 py-4 text-[10px] font-black text-zinc-500 uppercase tracking-widest w-24 text-center">Bloqueado</th>
                            <th className="px-6 py-4 text-[10px] font-black text-zinc-500 uppercase tracking-widest">Nombre del Usuario</th>
                            ...
                            {type === 'restaurant' && (
                                <>
                                    ...
                                    <th className="px-6 py-4 text-[10px] font-black text-zinc-500 uppercase tracking-widest text-center">Uso Cuentas</th>
                                </>
                            )}
                            <th className="px-6 py-4 text-[10px] font-black text-zinc-500 uppercase tracking-widest">Contraseña de Reinicio</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-800/50">
                        {filtered.length === 0 ? (
                            <tr>
                                <td colSpan={type === 'restaurant' ? 7 : 5} className="py-12 text-center text-zinc-500 font-bold text-sm uppercase tracking-widest">
                            ...
                            filtered.map(user => (
                                <tr key={user.id} className="hover:bg-zinc-900/40 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <p className="font-bold text-zinc-200 text-sm">{user.first_name} {user.last_name}</p>
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    {type === 'restaurant' && (
                                        <>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <span className="text-xs font-black bg-zinc-800 px-2 py-1 rounded-md text-zinc-300">
                                                    {user.usage_ratio}
                                                </span>
                                            </td>
                                        </>
                                        ...
                                        <div className="flex items-center gap-2 max-w-[200px]">
                                            <div className="relative flex-1">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full bg-zinc-900 border border-zinc-800 rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder:text-zinc-600 focus:outline-none focus:border-blue-500 transition-colors"
                                                />
                                            </div>
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-zinc-500 font-medium mt-1">Control de accesos y roles (PWA, Restorant y Administración)</p>
                </div>
            </div>

            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                    <TabsList className="bg-zinc-900/80 p-1 border border-zinc-800/50">
                ...
                <TabsContent value="pwa" className="animate-in fade-in-50 duration-300">
                    {loading ? <div className="p-8 text-center text-zinc-500">Cargando...</div> : renderTable('pwa', pwaUsers)}
                </TabsContent>

                <TabsContent value="restaurant" className="animate-in fade-in-50 duration-300">
                    {loading ? <div className="p-8 text-center text-zinc-500">Cargando...</div> : renderTable('restaurant', restaurantUsers)}
                </TabsContent>

                <TabsContent value="admin" className="animate-in fade-in-50 duration-300">
                    {loading ? <div className="p-8 text-center text-zinc-500">Cargando...</div> : renderTable('admin', adminUsers)}
                </TabsContent>
            </Tabs>
            ...
            {isAddModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
                    <div className="bg-zinc-950 border border-zinc-800 rounded-3xl w-full max-w-lg p-8 shadow-2xl relative animate-in zoom-in-95 duration-200">
                        <button
                            onClick={() => setIsAddModalOpen(false)}
                            className="absolute top-6 right-6 p-2 bg-zinc-900 border border-zinc-800 rounded-xl text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors"
                        >
                            <X className="w-4 h-4" />
                        </button>

                        <h2 className="text-xl font-black text-white px-2">Crear Nuevo Usuario</h2>
                        <p className="text-zinc-500 text-sm font-medium px-2 mb-6">Asigne los datos correspondientes para el nuevo acceso.</p>
                        ...
                        <form onSubmit={handleCreateUser} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        type="text"
                                        required
                                        className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:border-blue-500 focus:outline-none placeholder:text-zinc-700 font-medium"
                                        placeholder="Ej. Juan"
                            ...
                            <div className="space-y-2">
                                ...
                                <select
                                    className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:border-blue-500 focus:outline-none appearance-none font-medium"
                            ...
                            <div className="pt-4 flex gap-3">
                                <Button
                                    type="button"
                                    onClick={() => setIsAddModalOpen(false)}
                                    className="flex-1 bg-zinc-900 hover:bg-zinc-800 text-white font-bold tracking-widest uppercase text-xs h-12 rounded-xl"
                                >
                                    Cancelar
                                </Button>
                                <Button
