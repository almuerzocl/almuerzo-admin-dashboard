...
export default function LogsPage() {
    ...
    const getActionBg = (action: string) => {
        if (!action) return 'bg-zinc-800 border-zinc-700';
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-zinc-500 font-medium mt-1">Real-time system activity tracking</p>
                </div>

                <button className="bg-zinc-900 border border-zinc-800 text-zinc-300 px-6 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-zinc-800 transition-all shadow-lg hover:text-white">
            ...
            {/* Filters & Search */}
            <div className="bg-zinc-950/50 p-4 border border-zinc-800 rounded-3xl flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="relative w-full md:flex-1 md:max-w-md">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                    <input
                        type="text"
                        placeholder="Search logs..."
                        className="w-full bg-zinc-950/80 border border-zinc-800 rounded-xl py-3 pl-11 pr-4 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-blue-500 transition-all font-semibold"
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                    />
                </div>

                <div className="flex items-center gap-2 overflow-x-auto p-1 bg-zinc-900 rounded-xl border border-zinc-800">
                ...
                <div className="divide-y divide-zinc-800/50">
                    {loading ? (
                        Array(5).fill(0).map((_, i) => (
                            <div key={i} className="p-6 hover:bg-zinc-900/40 transition-colors flex items-start gap-4 animate-pulse">
                                <div className="mt-1 p-3 rounded-xl bg-zinc-800 border border-zinc-700 w-10 h-10" />
                                <div className="flex-1 space-y-3">
                                    <div className="h-4 bg-zinc-800 rounded w-1/3" />
                                    <div className="h-3 bg-zinc-800 rounded w-1/4" />
                                    <div className="h-16 bg-zinc-900 rounded w-full border border-zinc-800" />
                                </div>
                            </div>
                        ))
                        ...
                        filteredLogs.map((log) => (
                            <div key={log.id} className="p-6 hover:bg-zinc-900/40 transition-colors flex items-start gap-4 group">
                                <div className={cn(
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 gap-2">
                                        <p className="text-sm font-black text-white truncate font-mono">
                                            ...
                                            <span className="text-zinc-500 text-xs px-2 font-sans tracking-wide">on</span>
                                    ...
                                    <div className="flex items-center gap-2 text-xs font-semibold text-zinc-400 mb-4">
                                        ...
                                        <span className="px-2 py-1 rounded-md bg-zinc-800 text-zinc-300 capitalize text-[10px] font-black tracking-widest leading-none">
                                            {log.actor_type || 'system'}
                                        </span>
                                    </div>

                                    {/* Data Diff Preview */}
                                    <div className="bg-black/50 border border-zinc-800/80 rounded-xl p-4 font-mono text-[11px] text-zinc-400 overflow-x-auto shadow-inner">
                                        {log.action === 'UPDATE' ? (
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                                <div>
                                                    ...
                                                    <pre className="whitespace-pre-wrap leading-relaxed text-zinc-500">{JSON.stringify(log.old_data, null, 2)}</pre>
                                                </div>
                                                <div className="lg:border-l lg:border-dashed lg:border-zinc-800 lg:pl-6">
                                                    ...
                                                    <pre className="whitespace-pre-wrap leading-relaxed text-zinc-300">{JSON.stringify(log.new_data, null, 2)}</pre>
                                                </div>
                                            </div>
                                        ) : (
