        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
                <div>
                    ...
                    <p className="text-zinc-500 font-medium mt-1">Rendimiento global, ventas e indicadores del sistema</p>
                </div>

                <div className="flex flex-col sm:flex-row gap-3">
                    <div className="bg-zinc-950/80 p-1 rounded-2xl border border-zinc-800 flex items-center">
                        <button
                            ...
                            className={cn(
                                ...
                                timeRange === 'hoy' ? "bg-blue-600 text-white shadow-lg shadow-blue-900/20" : "text-zinc-500 hover:text-white"
                            )}
                        >
                            En Vivo: Hoy
                        </button>
                        <button
                            ...
                            className={cn(
                                "px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all",
                                timeRange === 'semana' ? "bg-blue-600 text-white shadow-lg shadow-blue-900/20" : "text-zinc-500 hover:text-white"
                            )}
                        >
                            Semana
                        </button>
                        <button
                            ...
                            className={cn(
                                "px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all",
                                timeRange === 'mes' ? "bg-blue-600 text-white shadow-lg shadow-blue-900/20" : "text-zinc-500 hover:text-white"
                            )}
                        >
                            Mes
                        </button>
                    ...
                    <div className="relative">
                        ...
                        {isSelectingCustom && (
                            <div className="absolute top-full mt-2 right-0 w-64 bg-zinc-950 border border-zinc-800 rounded-2xl p-4 shadow-2xl z-50 animate-in fade-in slide-in-from-top-2">
                                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-3 hover:text-white transition-colors">Seleccionar Fechas (Demo)</p>
                                <div className="space-y-2">
                                    <input type="date" className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-2 text-xs text-white color-scheme-dark" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                {[
                    ...
                    <div key={i} className={cn("bg-zinc-950/60 border border-zinc-800 p-6 rounded-[2rem] hover:border-zinc-700 transition-all group", loading ? "opacity-50" : "opacity-100")}>
                        ...
                        <div>
                            <p className="text-3xl font-black tracking-tighter text-white mb-1 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-zinc-400 transition-all">
                                {stat.val}
                            </p>
                            <p className="text-xs font-bold text-zinc-500 tracking-wide">{stat.label}</p>
                        </div>
                    </div>
                ))}
            </div>
